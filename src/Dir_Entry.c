...
struct Dir_Entry *deserialize_de(char *buffer) {
    ...
    unsigned long *fileLBAaddresses = malloc(sizeof(unsigned long) * (result->numFiles*2));
    memset(fileLBAaddresses, 0, sizeof(unsigned long) * (result->numFiles*2));
